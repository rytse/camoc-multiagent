...
def g_constr(obs_mfd):
    ...
    breakpoint()
    angle_to_targets = jnp.reshape(
        angle_to_targets, [2, int(angle_to_targets.shape[0] / 2)]
    )
    ...
    speed_to_agents = jnp.reshape(
        speed_to_agents, [3, int(speed_to_agents.shape[0] / 3)]
    )
    ...
    all_constrs = np.zeros(
        angle_constr.shape[0] + speed_constr_alpha.shape[0] + speed_constr_beta.shape[0]
    )
    all_constrs[: angle_constr.shape[0]] = angle_constr
    all_constrs[
        angle_constr.shape[0] : angle_constr.shape[0] + speed_constr_alpha.shape[0]
    ] = speed_constr_alpha
    all_constrs[
        angle_constr.shape[0] + speed_constr_alpha.shape[0] :
    ] = speed_constr_beta
