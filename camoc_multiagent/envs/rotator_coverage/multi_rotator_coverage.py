...
class MultiRotatorCoverageWorld:
    """
...
class MultiRotatorCoverageEnv:
    def __init__(self, n_agents: int, n_landmarks: int, n_worlds: int):
        self.device = torch.device("cuda")
        self.n_agents = n_agents
        self.n_landmarks = n_landmarks
        self.n_worlds = n_worlds
        self.world = MultiRotatorCoverageWorld(
            self.n_agents, self.n_landmarks, self.n_worlds
        )
        self.max_cycles = 100  # Just a random default all our agents "finish" at the same b/c we're not retarded
    ...
    def step(self, actions: torch.Tensor):
        ...
        return (
            ...
            self.world.global_reward(),
            self.current_step_mod_max_cycles == 0,
        )
    ...
    def render(self, mode="human"):
        ...
        if self.render_geoms is None:
            ...
            for i, entity in reversed(list(enumerate(self.world.positions[0]))):
